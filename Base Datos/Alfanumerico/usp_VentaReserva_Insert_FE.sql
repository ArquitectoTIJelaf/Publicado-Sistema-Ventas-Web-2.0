...
BEGIN TRY
		...
		If Exists(Select Top 1 1 from Venta Where SERIE_BOLETO=@Serie_Boleto and NUME_BOLETO=@Nume_Boleto 
			...
			Begin
				Select @Nume_Boleto=(Isnull(Max(NUME_BOLETO),0)+1) from Venta 
		...
		@Nit_Cliente,(@Tipo + @Serie_Boleto),@Nume_Boleto, '999', @Codi_Empresa, @Flag_Venta,convert(varchar(10),getdate(),103),'999',@Tipo_Doc, @Tipo,
		...
		from Venta 
		Where SERIE_BOLETO=(@Tipo + @Serie_Boleto) and 
		CODI_EMPRESA=@Codi_Empresa and 
		tipo=@Tipo and 
		nume_boleto=@Nume_Boleto
		
		if(ISNULL(@Id_venta,0)=0)
			Begin
		...
		values( @Id_venta,((@Tipo + @Serie_Boleto) + '-' + Right(('000000'+Cast(@Nume_Boleto as Varchar(6))),7)),@FechaViaje,@HoraViaje,@Codi_Servicio,
		...
		SET @POS=6;
		select Error,Cabecera,datosAdicionales,Detalle,Receptor,@Nume_Boleto Numero_Documento ,@Id_venta Id_Venta,(@Tipo + @Serie_Boleto) Serie_Documento from @Table
